...
const UserRecord = () => {
    ...
    const [userDataForEdit, setUserDataForEdit] = useState({id: "", uname: "", npwd: "", cnpwd: ""})
    ...
    const handleClickOpen = (id) => {
        ...
        setTimeout(()=>{setIsEditModalOpen(true)},500)
      };
    ...
    const getUserAccountById = async (id) => {
        ...
        const response = await Api().get("/get-user-account", { params: { id: id } })
    ...
    const deleteUserById = async (id) => {
        try{
            ...
            if(response.status === 200){
                ...
                setResponseMessage({status: "success" , message: response.data.success})
                setOpenSnackbar({ open: true });
             }
        } catch(err){
            setResponseMessage({status: "error" , message: "Something went wrong, try again!"})
            setIsDeleteModalOpen(false)
            setOpenSnackbar({ open: true });
        }
    ...
    const columns = useMemo(()=> [
        ...
        },
        {
            ...
            header: 'Created on',
            Cell: ({cell}) => {
                return <span className="badge bg-success">{dayjs(cell.getValue()).format('MMM DD,YYYY')}</span>
            }
        },
        {
           accessorKey: 'id',
           ...
           Cell: ({cell}) => {
                return <Box sx={{ display: 'flex', gap: '1rem' }}>
                            ...
                            <IconButton onClick={()=>{handleClickOpenDeleteModal(cell.getValue())}}>
    ...
    const onSubmitEditUser = async (data) => {
        try{
            ...
            if(response.status === 200){
                ...
                handleClose()
                setResponseMessage({status: "success" , message: "Updated Successfully"})
                setOpenSnackbar({ open: true });
             }
        } catch(err){
            setResponseMessage({status: "error" , message: "Something went wrong, try again!"})
        ...
        <Layout>
            <div>
                <div className='d-flex align-items-center justify-content-end mb-2 mt-5'>
                    <button className='btn btn-success' onClick={()=>setIsCreateModalOpen(true)}>+ New User</button>
                </div>
               <DataTable data={data} columns={columns} loading={loading}/>
                ...
                >
                    <CreateUser register={registerForm1} errors={errorsForm1} control={controlForm1}/>
                </CommonModal>
                <Snackbar open={open} autoHideDuration={3000} onClose={handleCloseSnackbar}>
                    <Alert onClose={handleCloseSnackbar} severity={responseMessage.status} sx={{ width: '100%' }}>
