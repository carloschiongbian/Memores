...
const Header = ({ handleLeftNavigation, isLeftNavigationOpen }) => {
    ...
    console.log(isLeftNavigationOpen)
    ...
    return (
        <nav id="nav" className="navbar navbar-expand-lg navbar-dark bg-primary fixed-top" style={{ "transition": "margin-left 0.5s", "marginLeft": isLeftNavigationOpen ? "250px" : "0px" }}>
            <div className="container-fluid text-white">

                <div className="d-flex">
                    {/* Button for Left Navigation Menu */}
                    <button className="btn btn-primary me-4" onClick={() => handleLeftNavigation()}><i className={`bi ${isLeftNavigationOpen ? 'bi-chevron-double-left' : 'bi-chevron-double-right'}`}></i></button>
                    <a href="" className="navbar-brand text-white">{title}</a>
                </div>
            </div>
        </nav>
    );
